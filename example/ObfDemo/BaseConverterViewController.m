...
@interface BaseConverterViewController () <UITextFieldDelegate>
...
- (void)setupLayout {
    ...
    headline.adjustsFontForContentSizeCategory = YES;
    ...
    subhead.adjustsFontForContentSizeCategory = YES;
    ...
    self.inputField.accessibilityLabel = @"Number input";
    ...
    self.baseSelector.accessibilityLabel = @"Source base selector";

    self.validationLabel = [[UILabel alloc] init];
    self.validationLabel.translatesAutoresizingMaskIntoConstraints = NO;
    self.validationLabel.textColor = [UIColor colorWithRed:1.0 green:0.8 blue:0.45 alpha:1.0];
    self.validationLabel.font = [UIFont systemFontOfSize:12 weight:UIFontWeightMedium];
    ...
    [cardContent addSubview:self.validationLabel];
    ...
    [NSLayoutConstraint activateConstraints:@[
        ...
        [self.validationLabel.leadingAnchor constraintEqualToAnchor:cardContent.leadingAnchor],
        [self.validationLabel.trailingAnchor constraintEqualToAnchor:cardContent.trailingAnchor],
        [self.validationLabel.topAnchor constraintEqualToAnchor:self.baseSelector.bottomAnchor constant:8],
        ...
        [self.resultsStack.topAnchor constraintEqualToAnchor:self.validationLabel.bottomAnchor constant:12],
...
- (NSString *)sanitize:(NSString *)text forBase:(NSInteger)base removed:(NSUInteger *)removedCount {
...
- (void)updateResults {
    ...
    NSString *sanitized = [self sanitize:rawInput forBase:sourceBase removed:&removedCount];

    if (![rawInput isEqualToString:sanitized]) {
        self.inputField.text = sanitized;
    }

    self.validationLabel.text = removedCount > 0 ? [NSString stringWithFormat:@"Removed %lu invalid character(s) for base %ld.", (unsigned long)removedCount, (long)sourceBase] : @"";
