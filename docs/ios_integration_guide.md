...
> 稳定性保护：`split/bogus`（实验性 CFG 扰动）默认关闭，可通过 `OBF_ENABLE_EXPERIMENTAL_CFG=1` 手动开启。
...
### Q18: `The sandbox is not in sync with the Podfile.lock`
这是 CocoaPods 工作区状态与锁文件不一致导致。请在示例目录执行：
...
- **Debug / O0 验证**：`my-clang`/`my-clang++` 在插件开启时会自动补 `-Xclang -disable-O0-optnone`（可用 `MY_CLANG_KEEP_OPTNONE=1` 关闭）；手动 clang 场景仍建议显式加上该参数。
