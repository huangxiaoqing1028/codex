...
> This project now uses the official Gradle Wrapper (`gradlew`) pinned to Gradle 8.0.


### Wrapper bootstrap (binary-free repo)
This repository intentionally does **not** commit `gradle-wrapper.jar` to avoid binary-file restrictions in some branch update flows.

After clone, run once to regenerate official wrapper files locally:
```bash
cd AndroidBaseConverter
gradle -b wrapper-bootstrap.gradle wrapper --gradle-version 8.0 --no-validate-url
```
Then use:
```bash
